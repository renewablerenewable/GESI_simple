from pyomo.core import AbstractModel, Set, Param, Var, Objective, NonNegativeReals, Binary

from gesi_model_web.model_component import *


def create_2050_model(name='gesi'):
...
def define_variable(M):
    ...
    # Binary Variable
    M.bi_battery_ch = Var(M.t, within=Binary)
    M.bi_battery_dis = Var(M.t, within=Binary)
...
def define_constraints(M):
    ...
    # New Binary Equation
    M.binary_charge = Constraint(M.t, rule=binary_charge_rule)
    M.binary_discharge = Constraint(M.t, rule=binary_discharge_rule)
    M.binary_decision = Constraint(M.t, rule=binary_decision_rule)

    # Emission Constraints
    M.em_limit = Constraint(rule=em_limit_rult_2050)
...
def define_objective(M):
    ...
    """
    M.total = Objective(rule=total_rule)
